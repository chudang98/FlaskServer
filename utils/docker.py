...
def create_container(project_id, twitter_url, email, table_id='canvas-figure-378911.twitter_crawl.tweet'):
  ...
  client_docker.containers.run(
    'crawl_app',
    f'--project_url {twitter_url} --table_id {table_id} --project_id {project_id} --email {email} --name {project_name}_{email}_{datetime.datetime.now(tz).strftime("%y%m%d%H%M%S")}',
    detach=True,
